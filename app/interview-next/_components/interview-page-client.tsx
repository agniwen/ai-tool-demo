...
export default function InterviewPageClient() {
  ...
  const handleResumeFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/interview-next/parse-resume', {
        method: 'POST',
        body: formData,
      });
  ...
  async function startInterview() {
    ...
    try {
      ...
      const tokenResponse = await fetch('/api/interview-next/token', { cache: 'no-store' });
                  ...
                  <div className='flex items-center gap-2'>
                    {isSessionPending
                      ...
                      : session?.user
                        ...
                        : <GoogleSignInButton callbackURL='/interview-next' />}
                  </div>
                )
              : session?.user
                ? (
